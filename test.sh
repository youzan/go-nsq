...
nsqlookupd --alsologtostderr=false --log-level=2 >$LOOKUP_LOGFILE 2>&1 &
LOOKUPD_PID=$!
sleep 10
...
nsqd --sync-timeout=100ms --broadcast-address=127.0.0.1 --alsologtostderr=true --log-level=1 --data-path=$DATAPATH --lookupd-tcp-address=127.0.0.1:4160 --tls-cert=./test/server.pem --tls-key=./test/server.key --tls-root-ca-file=./test/ca.pem >$NSQD_LOGFILE 2>&1 &
NSQD_PID=$!
